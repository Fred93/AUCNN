...
def exchangepoint(newPopulation, i, randompoint, nrows, ncolum):
    ...
    newPopulation[i].firstLevelMatrix()=newPopulation[i].firstLevelMatrix().reshape((nrows, ncolum))
    newPopulation[i+1].firstLevelMatrix()=newPopulation[i+1].firstLevelMatrix().reshape((nrows, ncolum))
    ...
    # reshape matrix with 1 row into original shape
    newPopulation[i].secondLevelMatrix()=newPopulation[i]secondLevelMatrix().reshape((nrows, ncolum))
    newPopulation[i+1].secondLevelMatrix()=newPopulation[i + 1].secondLevelMatrix().reshape((nrows, ncolum))
