...
class Chromosome:
    ...
    def calculateOutput(self, data):
        ...
        #using apply function is too expensive --> reshaping improves performance
        n = hiddenLayer_input.shape[0]
        m = hiddenLayer_input.shape[1]
        vec = hiddenLayer_input.reshape((1,n*m))
        vec = self.sigm(vec)
        #vec = np.apply_along_axis(self.sigmMath, 0, vec)
        hiddenLayer_output = vec.reshape((n,m))
        ...
        #print "step2: " + str(timeit.default_timer() - start)
