...
def calculateOutput(population, data):
    input = np.mat(data)
    input = np.append(np.mat(1), input, axis=1)                                         #add bias unit

    hiddenLayer_input = input * population.firstLevelMatrix
    ...
    outputLayer_input = hiddenLayer_output * population.secondLevelMatrix
